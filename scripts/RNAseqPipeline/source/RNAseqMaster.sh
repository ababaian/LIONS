...
echo "/home/jlever/bin/RNAseqMaster.sh <1: bam file (long path)> <2: name> <3: folder with all output (will be this PATH/name)> <4: species (hg19v66/...)> <5: strand specific(S)/regular(R)> <6: quality threshold> <7: running mask COVERAGE,RPKM,LEAKAGE,PROFILE,REPORT (1==run, 0==don't run))> [<8: java>]"
echo ""
...
if [ "$sr" == "S" ]; then
	if [ -s $Wdir"/"$name".q"$QC".F516.pos.wig.gz" ] && [ -s $Wdir"/"$name".q"$QC".F516.neg.wig.gz" ]; then
		echo "Wig files (pos and neg strand) exist"
	else	
		date > $Wdir/$name.wig.log
		echo "$J -jar -Xmx10G $JAVA_BASE/BAM2WIG.jar -bamFile $name".bam" -out $Wdir -q $QC -F 516 -s -samtools $SAMTOOLS -chr $chrfile >> $Wdir/$name.wig.log"
		$J -jar -Xmx10G $JAVA_BASE/BAM2WIG.jar -bamFile $name".bam" -out $MRWdir -q 0 -F 516 -s -samtools $SAMTOOLS -chr $chrfile >> $Wdir/$name.wig.log
		$J -jar -Xmx10G $JAVA_BASE/BAM2WIG.jar -bamFile $name".bam" -out $Wdir -q $QC -F 516 -s -samtools $SAMTOOLS -chr $chrfile >> $Wdir/$name.wig.log
	fi
else
	if [ -s $Wdir"/"$name".q"$QC".F516.wig.gz" ]; then
		echo "Wig file exist"
	else	
		date > $Wdir/$name.wig.log
		echo "$J -jar -Xmx10G $JAVA_BASE/BAM2WIG.jar -bamFile $name".bam" -out $Wdir -q $QC -F 516 -samtools $SAMTOOLS -chr $chrfile >> $Wdir/$name.wig.log"
		$J -jar -Xmx10G $JAVA_BASE/BAM2WIG.jar -bamFile $name".bam" -out $MRWdir -q 0 -F 516 -samtools $SAMTOOLS -chr $chrfile >> $Wdir/$name.wig.log
		$J -jar -Xmx10G $JAVA_BASE/BAM2WIG.jar -bamFile $name".bam" -out $Wdir -q $QC -F 516 -samtools $SAMTOOLS -chr $chrfile >> $Wdir/$name.wig.log
	fi
fi	
...
if [ "$RUN_RPKM" == "1" ]; then
	echo "RPKM..."
	$SHELL_BASE/RPKM.sh $name $length G A $species S
else
	echo "Skipping RPKM..."
fi
