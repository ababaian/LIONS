...
usage()
	...
	echo ""
	echo "USAGE:"
	echo "RPKM.sh <Project_name> <read_length> <Exon_type> <Metric> <reference_set> <Read Type>"
	echo "     <Exon Type> G = Gene, T = Transcripts"
	echo "     <Metric> A= Average Coverage, M= Median Coverage, X= 95% value of coverage"
	echo "     <Read Type> S = stranded, R = paired-end (regular)"
	echo ""
	echo "OUTPUT FORMAT:"
...
#INPUT PARAMETERS ----------------------------------------------
	name=$1 # Project Name
	length=$2 # Read Length
	type=$3 # G = Gene; T = transcript
	metric=$4 # Return Metic
