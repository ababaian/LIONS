...
# <Strand>: True = postive Strand, False = Negative strand
	...
	for read in readIterator:
		...
		if (read.is_paired and read.tid==read.rnext and int(read.mapq)>0):
			...
			elif ( MateStart - ReadLength > int(Coords[2])): # pair downstream
...
# Pre-computed Annotation Data (repeat)
	...
	if (ExonFile != "assembly_exons_2") and (file_exists(resTreeExon) and file_exists(resListExon) and file_exists(resInfoExon)):
