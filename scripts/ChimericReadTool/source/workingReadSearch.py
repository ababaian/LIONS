...
def processReads(samfile_path, exonTrees, repeatTrees, chimericBedFile):
	...
	for read in readIterator:
		...
		# Check if read is in proper pair (map to same chr)
...
# Pre-computed Annotation Data (repeat)
	...
	if (file_exists(resTreeExon) and file_exists(resListExon) and file_exists(resInfoExon)):
	...
	# Upstream Exon Coordinates
		...
		if ExonRank == 1: # First Exon
			...
			# Upstream Exon Rank (and python 0 index) 
			UpRank = ExonRank - 2

			UpExon = transcript[UpRank]
			
			# Extract upstream exon coordinates
			UpStart = UpExon[1]
			UpEnd = UpExon[2]
