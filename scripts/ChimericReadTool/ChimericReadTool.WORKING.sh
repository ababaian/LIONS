...
# Parse Exon File to contain annotation on single-exon transcripts
	...
	then
	 	...
	 	sh $SCRIPT_BASE/exon_scan.sh $exons
	fi
...
# Defines ER/DR/etc reads and aggrogates results
	...
	echo ""
	echo "$python3 $SCRIPT_BASE/chimericReadSearch.py "$exons"_2 $repeats $bam tmp.bed > $input"
	echo ""
	
	$python3 $SCRIPT_BASE/chimericReadSearch.py "$exons"_2 $repeats $bam tmp.bed > $input
