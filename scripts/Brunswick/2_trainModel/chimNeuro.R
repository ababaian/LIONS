# chimNeuro.R - <ER_Interaction> <Brunswick Directory>
#
...
# TRAINING or DATA EXPLORATION PROTOCOL
  TRAIN = T # run training [T] or run visualization [F]
    ...
    THRESH = 0.001 # Threshold for model cutoff [0.001]
    REPS = 1 # number of models to train per file
    SINGLE = T # Single Iteration or Infinite loop of training?
  
  RAND_START = T # Random initial weights (T) or known start model (F)
  # For refining an ANN model, set this to False and show the script
  # where the pre-trained ANN model is below
  # otherwise ignore
    ANN_TRAINED = 'ANN_INTERACTION_RUNID.Rdata' # If you have a starting model, you
